...
import { Button, Dialog, useToast, type ListView } from '@/app/crm/_components/ui'
...
export function WebinarTable({ webinars, view = 'table' }: { webinars: WebinarWithStats[]; view?: ListView }) {
  ...
  return (
    <>
      <Dialog
        ...
        description={webinarToDelete ? `Eliminar "${webinarToDelete.title}". Se perderán todos los registros.` : undefined}
        variant="danger"
      ...
      <div className={`${TOK.panel} p-6`}>
        ...
        {webinars.length === 0 ? (
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {webinars.map((w) => (
              <div
                ...
                className="flex min-h-48 cursor-pointer flex-col justify-between rounded-[var(--radius-lg)] bg-[var(--color-surface-container-lowest)] p-5 transition hover:bg-[var(--color-surface-container-low)]"
              >
                <div>
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="truncate text-sm font-semibold text-[var(--color-on-surface)]">{w.title}</p>
                      {w.platform && <p className={`mt-1 text-xs ${TOK.textSubtle}`}>{w.platform}</p>}
                    </div>
                    <span className={`text-xs ${TOK.textSubtle}`}>{formatDateShort(w.date)}</span>
                  </div>
                  {w.description && (
                    <p className={`mt-4 line-clamp-2 text-sm leading-5 ${TOK.textMuted}`}>{w.description}</p>
                  )}
                </div>
                <div className="mt-5 flex items-end justify-between gap-3">
                  <div className="grid grid-cols-3 gap-3 text-center">
                    <div>
                      <p className="text-sm font-bold text-[var(--color-primary)]">{w.registrations.length}</p>
                      <p className={`text-[10px] uppercase tracking-[0.07em] ${TOK.textSubtle}`}>Reg.</p>
                    </div>
                    <div>
                      ...
                      <p className={`text-[10px] uppercase tracking-[0.07em] ${TOK.textSubtle}`}>Asist.</p>
                    </div>
                    <div>
                      <p className="text-sm font-bold text-[var(--color-tertiary)]">
                        {w.registrations.filter((r) => r.status === 'PURCHASED').length}
                      </p>
                      <p className={`text-[10px] uppercase tracking-[0.07em] ${TOK.textSubtle}`}>Compró</p>
                    </div>
                  </div>
                  <div className="flex items-center justify-end gap-1" onClick={(e) => e.stopPropagation()}>
                    <button
                      onClick={(e) => { e.stopPropagation(); setEditWebinar(w) }}
                      className="cursor-pointer rounded-[var(--radius-sm)] border-none bg-transparent p-1.5 text-[var(--color-on-surface-variant)] transition-colors hover:bg-[var(--color-surface-container-high)]"
                      aria-label="Editar"
                    >
                    ...
                    <button
                      ...
                      className="cursor-pointer rounded-[var(--radius-sm)] border-none bg-transparent p-1.5 text-[var(--color-on-surface-variant)] transition-colors hover:bg-[var(--color-error-container)] hover:text-[var(--color-on-error-container)]"
                      aria-label="Eliminar"
                    >
                      <svg width="13" height="13" viewBox="0 0 16 16" fill="currentColor">
          ...
          <div>
            {/* Column headers */}
            <div className={`grid grid-cols-[2fr_1fr_0.6fr_0.6fr_0.6fr_0.4fr] px-4 pb-3 text-[10.5px] font-semibold uppercase tracking-[0.07em] ${TOK.textSubtle}`}>
            ...
            {webinars.map((w) => (
              <div key={w.id} onClick={() => router.push(`/crm/webinars/${w.id}`)}
                className="mb-1.5 grid grid-cols-[2fr_1fr_0.6fr_0.6fr_0.6fr_0.4fr] cursor-pointer items-center rounded-[var(--radius-md)] bg-[var(--color-surface-container-lowest)] px-4 py-3 transition last:mb-0 hover:bg-[var(--color-surface-container-low)]">
                <div>
